  ...
  module.exports = function(KEY, BYTES, wrapper, CLAMPED){
    ...
    if(FORCED){
      TypedArray = wrapper(function(that, data, $offset, $length){
        ...
        var index  = 0
          , offset = 0
          , buffer, byteLength, length, className;
        if(!isObject(data)){
          length     = strictToLength(data, true)
          byteLength = length * BYTES;
          buffer     = new $ArrayBuffer(byteLength);
        } else if(data instanceof $ArrayBuffer || (className = classof(data)) == ARRAY_BUFFER || className == SHARED_BUFFER){
          buffer = data;
      ...
      TypedArray = wrapper(function(that, data, $offset, $length){
        ...
        if(data instanceof $ArrayBuffer || (className = classof(data)) == ARRAY_BUFFER || className == SHARED_BUFFER){
...
$export($export.P + $export.F * _dereq_(34)(function(){
  ...
  slice: function slice(begin, end){
    ...
    if(className == 'Array')return arraySlice.call(this, begin, end);
    var start  = toIndex(begin, len)
      , upTo   = toIndex(end, len)
      , size   = toLength(upTo - start)
      , cloned = Array(size)
      , i      = 0;
    for(; i < size; i++)cloned[i] = className == 'String'
      ? this.charAt(start + i)
      : this[start + i];
    return cloned;
  }
